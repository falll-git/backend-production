const service = require("./digitalDocumentLoans.service");
...
exports.create = async (req, res) => {
  try {
    ...
    return res.status(201).json({
      status: true,
      success: true,
      message: "Permohonan peminjaman dokumen berhasil dibuat",
      data: result,
    });
  } catch (error) {
    return res.status(error.statusCode || 400).json({
...
exports.approve = async (req, res) => {
  try {
    ...
    return successResponse(
      res,
      result,
      "Permohonan peminjaman berhasil disetujui",
    );
  } catch (error) {
    return res.status(error.statusCode || 400).json({
      status: false,
...
exports.handover = async (req, res) => {
  try {
    ...
    return successResponse(res, result, "Penyerahan dokumen berhasil dicatat");
  } catch (error) {
    return res.status(error.statusCode || 400).json({
      status: false,
      success: false,
...
exports.returnLoan = async (req, res) => {
  try {
    ...
    return successResponse(
      res,
      result,
      "Pengembalian dokumen berhasil dicatat",
    );
  } catch (error) {
    return res.status(error.statusCode || 400).json({
      status: false,
