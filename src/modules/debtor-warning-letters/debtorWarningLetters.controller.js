const service = require("./debtorWarningLetters.service");
...
exports.create = async (req, res) => {
  try {
    const data = await service.create({ req, payload: req.body, userId: req.user?.id });
    return res.status(201).json({ status: true, success: true, message: "Surat peringatan berhasil dibuat.", data });
  } catch (error) {
    return res.status(status(error)).json({ status: false, success: false, message: error.message });
  }
};
...
exports.update = async (req, res) => {
  try {
    return successResponse(
      res,
      await service.update({ req, id: req.params.id, payload: req.body, userId: req.user?.id }),
